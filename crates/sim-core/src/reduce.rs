...
use crate::board::{LINK_ID_MASK, LINK_NEG_BIT};

/// Gather up to 64 *effective* (negation-applied) input bits starting at `inputs[base]` into a word:
/// bit `i` is link `inputs[base + i]`'s value XOR the pin's negate flag, which rides the top bit of
/// the same packed input-CSR word. Returns the word and how many bits it holds (`< 64` only for the
/// final partial chunk); bits above the count are zero, which lets `OR`/`XOR` ignore the count and
/// `AND` mask by it. A no-negation board has clear top bits, so this is the plain value gather and
/// the `^0` identity holds.
#[inline]
fn gather_eff(inputs: &[u32], ls: &BitSet, base: usize) -> (u64, u32) {
    let n = (inputs.len() - base).min(64);
    let mut w = 0u64;
    for i in 0..n {
        let raw = inputs[base + i];
        // The negate flag rides the same word as the link id, so it is folded in per pin — no
        // separate negate-bitset load alongside the (already scattered) value gather.
        let bit = ls.get(raw & LINK_ID_MASK) ^ (raw & LINK_NEG_BIT != 0);
        w |= (bit as u64) << i;
    }
    (w, n as u32)
}
...
pub(crate) fn and_inputs(inputs: &[u32], ls: &BitSet) -> bool {
    ...
    while base < inputs.len() {
        let (w, n) = gather_eff(inputs, ls, base);
...
pub(crate) fn or_inputs(inputs: &[u32], ls: &BitSet) -> bool {
    let mut base = 0;
    while base < inputs.len() {
        if gather_eff(inputs, ls, base).0 != 0 {
            return true;
        }
        base += 64;
    }
...
pub(crate) fn xor_inputs(inputs: &[u32], ls: &BitSet) -> bool {
    ...
    let mut base = 0;
    while base < inputs.len() {
        ones += gather_eff(inputs, ls, base).0.count_ones() as u64;
        base += 64;
    }
    ones & 1 == 1
}
...
mod tests {
    ...
    fn reductions_match_oracle_across_fanin_and_chunk_boundaries() {
        ...
        for &n in &fanins {
            ...
            for seed in [1u64, 0xDEAD_BEEF, 0x5555_5555_AAAA_AAAA, u64::MAX] {
                ...
                let inputs: Vec<u32> = (0..n as u32).collect(); // clear top bits = no negation
                assert_eq!(
    ...
    /// Negation rides the top bit of each packed input word; the gather folds it in per pin. Stress
    /// mixed negate patterns across the chunk boundaries against a per-pin scalar oracle
    /// (`value XOR negate`), including fan-ins straddling 64/128.
    #[test]
    fn negation_via_packed_top_bit() {
        for &n in &[1usize, 2, 4, 8, 10, 33, 63, 64, 65, 100, 129] {
            let bits = (n as u32).max(1);
            for seed in [1u64, 0xA5A5_1234_5678_9ABC, u64::MAX, 0x0F0F_F0F0_1357_2468] {
                let ls = seeded_bitset(bits, seed);
                // An independent per-pin negate pattern.
                let mut neg = vec![false; n];
                let mut x = seed ^ 0x9999_9999_9999_9999;
                for b in neg.iter_mut() {
                    x = crate::scratch::splitmix64(x);
                    *b = x & 1 == 1;
                }
                // Pin i reads link i, with the top bit set iff that pin is negated.
                let inputs: Vec<u32> = (0..n as u32)
                    .map(|i| if neg[i as usize] { i | LINK_NEG_BIT } else { i })
                    .collect();
                let eff = |i: usize| ls.get(i as u32) ^ neg[i];
                let oand = (0..n).all(eff);
                let oor = (0..n).any(eff);
                let oxor = (0..n).filter(|&i| eff(i)).count() & 1 == 1;
                assert_eq!(and_inputs(&inputs, &ls), oand, "AND n={n} seed={seed}");
                assert_eq!(or_inputs(&inputs, &ls), oor, "OR n={n} seed={seed}");
                assert_eq!(xor_inputs(&inputs, &ls), oxor, "XOR n={n} seed={seed}");
            }
        }
    }
}
