...
//
// Time-bound by default (count the ticks that land in `--ms`); `--ticks N` forces a fixed step count.
//
// Usage: node bench-wasm.mjs <board.json> [--ms N | --ticks N] [--repeat R]
...
if (!boardPath) {
  console.error('usage: node bench-wasm.mjs <board.json> [--ms N | --ticks N] [--repeat R]');
  process.exit(2);
}
...
console.error(`benching ${name} via wasm blocking run() — ${label} × ${repeat} repeats`);
